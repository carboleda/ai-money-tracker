...
import { Frequency } from "../model/recurring-expense.model";
...
@Injectable()
export class ScheduleRecurringExpenseService {
  constructor(
    private readonly getAllRecurringExpensesService: GetAllRecurringExpensesService,
  ...
  async execute(): Promise<ScheduleResult> {
    ...
    const { recurringExpensesConfig } =
      await this.getAllRecurringExpensesService.execute();
  ...
  private getTransactionDate(
    recurringExpense: RecurringExpenseOutput
  ): Date | null {
    const now = new Date();
