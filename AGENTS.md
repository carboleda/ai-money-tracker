...
@injectable()
export class CreateTransactionService implements Service<CreateTransactionInput, Transaction> {
  constructor(
...
try {
  ...
  return NextResponse.json({ message: domainError.message }, { status: domainError.statusCode ?? 500 });
}
```
